...
export function importTagsFromCustomSearches(tagsObj, lookupReadableMetadataName) {
  if (tagsObj) {
    ...
    Object.keys(tagsObj).forEach((m) => { // for each tag
      ...
      if (vals && vals.length > 0) {
        ...
        updatedTagsObj[readableName] = Object.values(tagsObj[m]).map(a => ({
          name: readableName,
          tags_id: a,
          tag_sets_id: parseInt(m, 10),
          label: 'Pub Country',
          tag_set_label: 'Pub Country',
          tag_set_name: 'pub_country',
          selected: true,
          value: true,
        }));
      }
...
export function decodeQueryParamString(queryString, lookupReadableMetadataName) {
  ...
  if (queryString.query && queryString.query.tags) {
    queriesFromUrl.tags = importTagsFromCustomSearches(JSON.parse(queryString.query.tags), lookupReadableMetadataName);
  }
