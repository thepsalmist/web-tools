...
const PlatformDescription = (props) => {
  ...
  const rootClasses = `platform-description ${disabledClass} ${selectedClass}`;
  ...
  if (image) {
    ...
    visualContent = <div className="platform-icon" tabIndex={0} role="button" onKeyPress={clickHandler} onClick={clickHandler}>{icon()}</div>;
  }
  return (
...
export default injectIntl(PlatformDescription);
