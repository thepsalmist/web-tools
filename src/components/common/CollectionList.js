...
import { compareTagNames } from '../../lib/tagUtil';
import { DownloadButton } from './IconButton';
import messages from '../../resources/messages';
import { getUserRoles, hasPermissions, PERMISSION_MEDIA_EDIT } from '../../lib/auth';

const CollectionList = (props) => {
  const { title, intro, collections, onDownload, helpButton, user, linkToFullUrl, collectionSets } = props;
  ...
  const validCollections = collections ? collections.filter(c => (collectionSets.includes(c.tag_sets_id) && (c.show_on_media || canSeePrivateCollections))) : [];
